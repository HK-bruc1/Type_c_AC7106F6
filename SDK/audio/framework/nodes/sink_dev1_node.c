...
static int sink_dev1_init(struct sink_dev1_hdl *hdl)
{
	...
	usb_call_audio_tap_stream_start(USB_CALL_AUDIO_TAP_FAR,
							hdl->fmt.sample_rate,
							hdl->fmt.coding_type,
							ch_num,
							hdl->fmt.bit_wide,
...
{
	usb_call_audio_tap_stream_stop(USB_CALL_AUDIO_TAP_FAR);
    return 0;
}
...
{
	usb_call_audio_tap_input(USB_CALL_AUDIO_TAP_FAR, data, data_len);
}
