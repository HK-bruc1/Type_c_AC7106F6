        ...
        if (frame->offset == 0) {
            if (config_audio_dac_node_fade_out_en) {
                if (note->state & NODE_STA_DEC_PAUSE) {
                    int fade_out_en = jlstream_event_notify(STREAM_EVENT_GET_FADE_OUT_STATE, hdl->scene);
                    if (fade_out_en) {
                        //dac节点对末尾一帧数据做一次淡出处理，避免某些音效处理带有缓存效果，无法淡出到0的情况
                        int per_ch_npoints = frame->len / (hdl_node(hdl)->iport->prev->fmt.bit_wide ? 4 : 2) / AUDIO_CH_NUM(hdl_node(hdl)->iport->prev->fmt.channel_mode);
                        float step = 1.0f / per_ch_npoints;
                        hdl->fade_value = audio_fade_out((void *)frame->data, step, hdl->fade_value, AUDIO_CH_NUM(hdl_node(hdl)->iport->prev->fmt.channel_mode), per_ch_npoints, hdl_node(hdl)->iport->prev->fmt.bit_wide);
                    }
                }
            }
