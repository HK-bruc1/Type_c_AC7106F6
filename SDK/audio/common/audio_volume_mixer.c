...
#endif
#if (((defined TCFG_AUDIO_SPATIAL_EFFECT_ENABLE) && TCFG_AUDIO_SPATIAL_EFFECT_ENABLE) && \
    ((defined SPATIAL_AUDIO_EFFECT_SW_TONE_PLAY) && SPATIAL_AUDIO_EFFECT_SW_TONE_PLAY))
                //空间音效开关使用不同数据流，开：Vol_BtmMusic、关：Vol_BtsMusic
                if (!esco_player_is_playing(NULL)) { //播歌
                    if (get_a2dp_spatial_audio_mode() == SPATIAL_EFX_OFF) { //空间音效关闭状态
                        sprintf(node_name, "%s%s", "Vol_Bts", dvol_type[i]);
                    }
                }
#endif
