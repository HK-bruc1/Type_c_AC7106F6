...
{
#if RDX_MVP0_PRODUCT_IS_CHARGE_CASE
    int len;
    ...
    len = snprintf((char *)rdx_read_value, sizeof(rdx_read_value),
                   ...
                   RDX_MVP0_TEST_AUTH_KEY, RDX_MVP0_TEST_LABEL_SN);
    if (len < 0) {
...
#else
    rdx_read_value_len = sizeof(RDX_MVP0_TEST_AUTH_KEY) - 1;
    memcpy(rdx_read_value, RDX_MVP0_TEST_AUTH_KEY, rdx_read_value_len);
#endif
}
...
u8 rdx_identity_fill_manufacturer_data(u8 *buffer, u8 buffer_size)
{
    u8 offset = 0;
    u8 i;
    u32 ability = RDX_MVP0_DEVICE_ABILITY;
    const u8 required = (sizeof(RDX_MVP0_PRODUCT_CODE) - 1)
                        + RDX_MAC_SIZE
                        + (sizeof(RDX_MVP0_FACTORY_CODE) - 1)
                        + 1 + 4
#if RDX_MVP0_INCLUDE_PROTOCOL_VERSION
                        + 1
#endif
                        + (sizeof(RDX_MVP0_PRODUCT_TYPE) - 1)
                        + (sizeof(RDX_MVP0_SELF_MARK) - 1);

    if (!buffer || buffer_size < required) {
        return 0;
    }

    memcpy(buffer + offset, RDX_MVP0_PRODUCT_CODE, sizeof(RDX_MVP0_PRODUCT_CODE) - 1);
    offset += sizeof(RDX_MVP0_PRODUCT_CODE) - 1;

    for (i = 0; i < RDX_MAC_SIZE; i++) {
        buffer[offset++] = rdx_ble_mac[RDX_MAC_SIZE - 1 - i];
    }

    memcpy(buffer + offset, RDX_MVP0_FACTORY_CODE, sizeof(RDX_MVP0_FACTORY_CODE) - 1);
    offset += sizeof(RDX_MVP0_FACTORY_CODE) - 1;

    buffer[offset++] = RDX_MVP0_BOUND_STATE ? BIT(6) : 0;
...
#if RDX_MVP0_INCLUDE_PROTOCOL_VERSION
    buffer[offset++] = RDX_MVP0_PROTOCOL_VERSION;
#endif

    memcpy(buffer + offset, RDX_MVP0_PRODUCT_TYPE, sizeof(RDX_MVP0_PRODUCT_TYPE) - 1);
    offset += sizeof(RDX_MVP0_PRODUCT_TYPE) - 1;
    memcpy(buffer + offset, RDX_MVP0_SELF_MARK, sizeof(RDX_MVP0_SELF_MARK) - 1);
    offset += sizeof(RDX_MVP0_SELF_MARK) - 1;
